...
├── requirements.txt          # Lightweight dependencies for Vercel Serverless runtime
...
└── src/                      # Source package (local ChromaDB RAG pipeline)
    ├── __init__.py
    ...
    └── api.py                # Local FastAPI server using full ChromaDB retriever
```
...
- **Frontend Production Build**: `npm run build` executed successfully via Vite, compiling `dist/index.html`, `dist/assets/*.css`, and `dist/assets/*.js` in 3.5s with 0 errors.
